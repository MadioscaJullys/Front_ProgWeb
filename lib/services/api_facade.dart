...
class ApiFacade extends ChangeNotifier {
  // Observer Pattern: Extende ChangeNotifier para notificar mudanças
  ...
  ApiFacade(this._authService, this._userService, this._postsService) {
    // Injeção de dependências no construtor
    // Observer Pattern: A facade observa mudanças nos serviços para notificar seus próprios listeners
    _authService.addListener(
      notifyListeners,
    ); // Registra listener no AuthService
    _userService.addListener(
      notifyListeners,
    ); // Registra listener no UserService
  }

  // Service específico para posts (injetado)
  final PostsService _postsService;

  // === MÉTODOS DELEGADOS PARA AUTENTICAÇÃO (Facade simplifica acesso) ===

  bool get isAuthenticated => _authService
      .isAuthenticated; // Propriedade delegada: verifica se usuário está logado
  User? get currentUser =>
      _authService.currentUser; // Propriedade delegada: obtém usuário atual
  ...
  Future<bool> createPostScreen({
    ...
    try {
      ...
      debugPrint('createPostScreen error: $e');
      return false;
    }
  }

  Future<bool> login(String email, String password) =>
      _authService.login(email, password); // Método delegada: faz login
  ...
  Future<List<User>> getUsers() async {
    // Facade: Método unificado para buscar todos os usuários
    final token =
        _authService.token; // Obtém token do serviço de auth (autorização)
  ...
  Future<User?> getUser(int userId) async {
    // Facade: Método unificado para buscar usuário por ID
  ...
  Future<bool> createUser(UserCreate userCreate) async {
    // Facade: Método unificado para criar usuário
  ...
  Future<bool> updateUser(int userId, UserUpdate userUpdate) async {
    // Facade: Método unificado para atualizar
  ...
  Future<bool> deleteUser(int userId) async {
    // Facade: Método unificado para deletar usuário
    final token =
        _authService.token; // Verifica se usuário tem permissão para deletar
